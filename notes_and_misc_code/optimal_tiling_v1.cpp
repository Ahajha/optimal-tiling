...
void checkLoops()
{
	// This is a minor optimization: If a column can come immediately after itself, then
	// it comprises a tile, and we can use that immediately, then remove it from
	// the adjacency list, as it will never be used after that.
	
	for (unsigned i = 0; i < columns.size(); i++)
	{
		auto& cols = columns[i].adjList;
		
		std::erase_if(cols, [i](std::pair<unsigned, unsigned> item)
		{
			if (item.first == i)
			{
				tile t(i);
				...
				if (t > bestTile)
				{
					// Print a message and update bestTile
					std::cout << "found " << t.numVertices << '/' << n
						<< " (" << ((double)t.numVertices/n) << ")"
						<< std::endl << t << std::endl;
...
//const unsigned MAX_TILE_SIZE = 10;

// Recursive helper function to enumerateTiles.
void enumerateTilesRecursive(tile& t, std::vector<bool>& used)
{
	unsigned start = t.columnPath.front().first;
	unsigned currentEnd = t.columnPath.back().first;
	
	used[currentEnd] = true;
	
	for (auto& adj : columns[currentEnd].adjList)
	{
		// Minor inefficiency, we really only need to check if the first
		// item is the start.
		if (adj.first == start)
		{
			// 'Output' (check) the current tile as is
			if (t > bestTile)
			{
				// Print a message and update bestTile
				std::cout << "found " << t.numVertices << '/'
					<< (n * t.columnPath.size()) << " ("
					<< ((double)t.numVertices/(n * t.columnPath.size()))
					<< ")" << std::endl << t << std::endl;
				
				bestTile = t;
			}
		}
		// Only search if the column has not yet been used
		else if (!used[adj.first]/* && t.columnPath.size() < MAX_TILE_SIZE*/)
		{
			...
			enumerateTilesRecursive(t, used);
	...
	used[currentEnd] = false;
}

void enumerateTiles()
{
	// Used to mark which columns have been used. Starts with all false.
	std::vector<bool> used(columns.size());
	...
	for (unsigned i = 0; i < columns.size(); i++)
	{
		...
		// This column class is no longer needed, so remove it
		// from any class' adjacency lists that contain it.
		for (auto& colClass : columns)
		{
			if (colClass.adjList.front().first == i)
			{
				colClass.adjList.pop_front();
			}
		}
		
		if (trace)
		{
			std::cout << "New adjacency lists (only >" << i << "):" << std::endl;
			
			for (unsigned j = i + 1; j < columns.size(); j++)
			{
				std::cout << j << ':';
				
				for (auto& pair : columns[j].adjList)
