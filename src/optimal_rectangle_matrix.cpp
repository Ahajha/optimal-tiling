...
void parseArgs(int argn, char** args)
{
	if (argn < 1 || argn > 2)
	{
		std::cerr << "usage: " << args[0] << " [-t]" << std::endl;
		exit(1);
	}
	...
	for (unsigned len = 2; len < paths_info.lengths.size(); len++)
	{
		...
		for (unsigned end = 0; end < columns.size(); end++)
		{
			...
			for (const auto& neighbor : columns[end].adjList)
			{
				...
				// Check if this is the new best for this cell
				if (new_num_induced < old_num_induced + (int)columns[adj].cols[0].numVertices)
				{
					// The second pair needs to be reconstructed by extractRectangle
					...
					if (density > bestRectangles[len].density)
					{
						...
						if (trace) std::cout << "found (length " << len << "): "
...
void enumerate()
{
	bestRectangles.resize(n + 1);
	
	bestRectangles[1].density = fraction(n,n);
...
int main(int argn, char** args)
{
	...
	// 13 is found to be a good stopping point for now
	for (n = 1; n <= 13; n++)
	{
		if (trace) std::cout << "n = " << n << std::endl;
		
		// This could just be done once, but isn't that much of a speed penalty
		produceConfigs();
		...
		for (unsigned i = 1; i < bestRectangles.size(); i++)
		{
			std::cout << bestRectangles[i].density.num << ' ';
		}
		std::cout << std::endl;
