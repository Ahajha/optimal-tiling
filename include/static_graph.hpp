...
requires(sizeof...(dims) >= 1 && ((dims > 0) && ...)) class static_hrp_graph {
...
public:
  ...
  [[nodiscard]] constexpr static vertex_id get_coord(std::size_t d,
                                                     vertex_id vid) {
  ...
  [[nodiscard]] constexpr static vertex_id forward(std::size_t d,
                                                   vertex_id vid) {
    return (get_coord(d, vid) == dims_array[d] - 1) ? no_vertex
                                                    : vid + size_of_dim(d);
  }

  // Returns the vertex id of the vertex "backward" in a
  // dimension d from c, if it exists, EMPTY if not.
  [[nodiscard]] constexpr static vertex_id backward(std::size_t d,
                                                    vertex_id vid) {
    return (get_coord(d, vid) == 0) ? no_vertex : vid - size_of_dim(d);
  }
